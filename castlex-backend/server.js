...
app.put("/api/posts/:id", auth, async (req, res) => {
  try {
    ...
    if (!content?.trim()) {
      return res.status(400).json({
        error: "متن پست نمی‌تواند خالی باشد",
      });
    }

    const { data: post, error: postError } = await supabase
      .from("posts")
      .select("id, user_id, created_at, post_type")
      .eq("id", postId)
      .maybeSingle();
    ...
    if (String(post.user_id) !== String(req.user.id)) {
      return res.status(403).json({
        error: "فقط صاحب پست می‌تواند آن را ویرایش کند",
      });
    }

    if (post.post_type === "poll") {
      return res.status(400).json({
        error: "نظرسنجی قابل ویرایش نیست",
      });
    }

    if (Date.now() - new Date(post.created_at).getTime() > 10 * 60 * 1000) {
      return res.status(403).json({
        error: "زمان ویرایش پست تمام شده است",
      });
    }
